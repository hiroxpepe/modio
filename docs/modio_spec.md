...
Checked against the code, 2026-08-22. **Read back and made new
again
2026-09-20** — a month of real work (`place_memory_design.md`,
`sight_checklist.md`, and `TASK-021`'s own real findings) had gone
by with this whole part never read back once against it.
...
| Zero-GC (`Recall`'s own table)     | **Done.** The table is a ring at a fixed size; tests count the bytes and find 0. **Not yet true of `Runtime`/`Sight` — see 9.2, #1**                                                                                                                                                        |
...
| What a Deed holds of what it found | **Done, checked live 2026-09-20 — read back wrong below for a month.** `TASK-019` built `Deed.Holding`, a `Choice` (`Kind`, `ID`, `Angle`, `Distance`, `Height`), carried from the seek through to the row written at the end. This was long marked open in 9.2 after it was already closed |
...
| 2 | How places are drawn               | `place_memory_design.md` (2026-09-19) now sets this out in full — a 4-by-3, 12-cell picture of wall-distance, each in one of four bands. **Still owed: no sums run on a real level (`Level_1`'s own count of true ways in), and where the band lines truly sit for a real `reach`.**                                                                                                                                                                                                                          |
| 3 | The cost at 64 characters          | Worked out: 3,200 wide checks a second, near 960 thrown lines, 37 KB of memory (§3.7). **Not measured on a real phone.**                                                                                                                                                                                                                                                                                                                                                                                      |
| 4 | How long a deed truly takes        | 30 seconds is the most one may run (§5.3), and nothing says how long one truly takes. Every persona number resting on that is a guess until it is measured.                                                                                                                                                                                                                                                                                                                                                   |
| 5 | Sound                              | `germio` holds `SoundSystem`, and `signo`/`quyno` stand behind it. **Nothing here calls any of them.** To be taken up later (Master's own word).                                                                                                                                                                                                                                                                                                                                                              |
| 6 | Reading a level's rules            | `germio`'s own `Grapher.RulesByActor` sorts them, but nothing calls it yet: a reader — a tool, a page, an editor — is owed.                                                                                                                                                                                                                                                                                                                                                                                   |
| 7 | `IMind` wired to a real `Engine`   | **Found true, `TASK-021`.** `animo.Engine` holds almost every member `IMind` asks for, but two do not truly match: `Lock` wants a plain `bool`, `Engine.Lock` takes a real `LockMode`; `Affect`'s own arity differs too. A thin `EngineMind` adapter is designed in full in `TASK-021`, but not one line of it stands built. No real `.asmdef` stands for `Modio` either (`TASK-023`)                                                                                                                         |
| 8 | The head, turned apart from a Deed | **Held in `sight_checklist.md` §4, 2026-09-20.** `Sight` moves to `germio`, not `Modio` (keeps the depend-on-nothing order true, and keeps the wedge check open to `dotnet test`). The head's own turn, while `Explore` runs, is `stemic`'s own work, wholly apart from any `Deed` — `Modio` never knows it runs. A real target's own `Choice.Angle` (already built, `TASK-019`) could feed it once `Runtime/` stands and truly fills a `Choice`; until then, only a made-up, `Sight`-free sweep may be built |
